...
const ChatSidebar = ({ onConversationSelect }) => {
  ...
  useEffect(() => {
    if (currentUser) {
      // Assuming userType is available in currentUser
      fetchUserChatRooms(currentUser.userId, currentUser.userType);
    }
  }, [currentUser, fetchUserChatRooms]);

  const handleRoomSelect = (room) => {
    joinRoom(room._id);
    // You might want to create a conversation object for the room
    const conversation = {
      roomId: room._id,
      name: room.name,
      isGroup: true
      ...
      <div className="conversation-list">
        {chatRooms.length === 0 ? (
          ...
          chatRooms.map((room) => (
            <div 
              key={room._id}
              className={`conversation-item ${currentRoom === room._id ? 'active' : ''}`}
              onClick={() => handleRoomSelect(room)}
            >
              <div className="conversation-avatar">
                <span>{room.name.substring(0, 2).toUpperCase()}</span>
              </div>
              <div className="conversation-details">
                <div className="conversation-name">
                  {room.name}
