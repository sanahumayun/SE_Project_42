...
const ChatWindow = ({ onBackClick, showBackButton }) => {
  ...
  const handleSendMessage = (e) => {
    ...
    if (selectedConversation.roomId) {
      // Send to room
      sendMessage(message, null, selectedConversation.roomId);
    } else {
  ...
  const getCurrentRoom = () => {
    if (!selectedConversation?.roomId) return null;
    return chatRooms.find(room => room._id === selectedConversation.roomId);
  };
  ...
  return (
    <div className="chat-window">
      <div className="chat-header">
        ...
        <div className="recipient-info">
          <h3>{selectedConversation.name}</h3>
          {room && <span className="room-topic">{room.topic}</span>}
        </div>
      </div>
